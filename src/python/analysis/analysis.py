...
if "exclude_zeros" not in config:
    raise ValueError("Missing 'exclude_zeros' entry in config.txt. Please specify exclude_zeros=True or False.")
exclude_zeros_val = config["exclude_zeros"].strip().lower()
if exclude_zeros_val not in ["true", "false"]:
    raise ValueError("Invalid value for 'exclude_zeros'. Must be 'True' or 'False'.")
zero_flag = "exclude_zeros" if exclude_zeros_val == "true" else "include_zeros"
...
input_filename = f"output_{datatype}_{bits}_bit_{zero_flag}.txt"
input_path = os.path.join(results_dir, "weights", input_filename)
